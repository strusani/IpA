...
var notSupportedBrowsers = [
  {'os': 'Any', 'browser': 'MSIE', 'version': 8},
	...
	{'os': 'Any', 'browser': 'Opera', 'version': 10.0}
];
...
var BrowserDetection = {
	init: function(){
		...
		window.console && console.log("detected: " + this.os + this.browser + this.browserVersion);
		
		if(this.browser == '' || this.browser == 'Unknown' || this.os == '' || 
		   this.os == 'Unknown' || this.browserVersion == '' || this.browserVersion == 0)
		{
			window.console && console.log("unknown browser - returning");
			return;
		}
		...
		if(oldBrowser){
			window.console && console.log("old browser detected - displaying notice");
			this.displayNotice();
		}
	},
	...
	noticeText: {
    	   'professional': { "title": "Unsupported Browser Detected", "notice": "This App has detected that you are using an unsupported browser. Using your current browser will prevent you from accessing some of the features of this app. An upgrade may not be required, but is strongly recommend to improve your browsing experience on our website.", "selectBrowser": "Use the links below to download a new browser or upgrade your existing browser.", "remindMeLater": "I'll try it once with my current browser", "neverRemindAgain": "No, don't remind me again" }
	}
};
